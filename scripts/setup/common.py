...
def wait_for_service(url, api_key, service_name, endpoint="/api/v3/system/status", header_name="X-Api-Key", max_retries=30, retry_delay=2):
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if attempt % 5 == 0:
            log(f"Still waiting for {service_name} (attempt {attempt}/{max_retries}): {last_error}", "WARNING")
        time.sleep(retry_delay)
    
    log(f"{service_name} not reachable after {max_retries} attempts. Last error: {last_error}", "ERROR")
    sys.exit(1)
...
def configure_download_clients(url, api_key, download_clients, api_version="v3", header_name="X-Api-Key"):
    ...
    for client_config in download_clients:
        ...
        if missing_env_vars:
            log(f"Download client {name} has missing environment variables: {', '.join(missing_env_vars)}", "WARNING")
