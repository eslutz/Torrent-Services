...
def login(username, password):
    try:
        ...
        resp = session.post(
            f"{QBIT_URL}/api/v2/auth/login", data={"username": username, "password": password}
        )
...
def update_credentials(session, new_user, new_pass):
    ...
    try:
        ...
        resp = session.post(
            f"{QBIT_URL}/api/v2/app/setPreferences", data={"json": json.dumps(payload)}
        )
...
def configure_preferences(session):
    ...
    try:
        resp = session.post(
            f"{QBIT_URL}/api/v2/app/setPreferences", data={"json": json.dumps(safe_prefs)}
        )
