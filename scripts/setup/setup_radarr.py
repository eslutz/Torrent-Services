...
import requests
from common import (
    load_env, load_config, log, get_api_key, get_headers,
    wait_for_service, disable_analytics,
    configure_config_endpoint, configure_root_folders
)
...
def main():
    ...
    configure_config_endpoint(
        RADARR_URL, api_key, "mediaManagement",
        RADARR_CONFIG.get("media_management"), "Media Management"
    )
    
    configure_config_endpoint(
        RADARR_URL, api_key, "naming",
        RADARR_CONFIG.get("naming"), "Naming"
    )
    
    configure_root_folders(
        RADARR_URL, api_key,
        RADARR_CONFIG.get("root_folders", [])
    )
