...
- **`setup_auth.py`**: Automates authentication setup all services (Gluetun, qBittorrent, Prowlarr, Sonarr, Radarr, Bazarr).
...
1. **Environment Variables**: The scripts rely on the `.env` file in the project root.
    - Gluetun: `GLUETUN_CONTROL_APIKEY` (auto-generated by `setup_auth.py` on first run).
    - API Keys: `PROWLARR_API_KEY`, `SONARR_API_KEY`, `RADARR_API_KEY`, `BAZARR_API_KEY`.
    - Credentials: `SERVICE_USER`, `QBITTORRENT_PASSWORD`, `PROWLARR_PASSWORD`, `SONARR_PASSWORD`, `RADARR_PASSWORD`, `BAZARR_PASSWORD`.
    - Secrets: e.g., `IPTORRENTS_COOKIE`.
2. **Python Dependencies**:
    - `requests`, `playwright`, and Docker CLI are required.

    ```bash
    pip install requests playwright
    playwright install chromium
    # Docker CLI must be available (already installed if using Docker Desktop)
    ```
...
- **Sets up Gluetun Control Server**: Generates API key using `docker run --rm qmcgaw/gluetun genkey` and updates `.env` with `GLUETUN_CONTROL_APIKEY`.
- **Sets up Authentication**: Uses Playwright to automatically set username and password for qBittorrent, Prowlarr, Sonarr, Radarr, and Bazarr based on `.env` credentials.
...
| Service | Auth source | Config location | .env variable |
|---------|-------------|------------------|---------------|
| Gluetun | API key (auto-generated) | Environment Variable | `GLUETUN_CONTROL_APIKEY` |
