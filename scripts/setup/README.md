...
1. **Environment Variables**: The scripts rely on the `.env` file in the project root.
    - Gluetun: `CONTROL_APIKEY` (auto-generated by `setup_auth.py` on first run).
...
- **Sets up Gluetun Control Server**: Generates API key using `docker run --rm qmcgaw/gluetun genkey` and updates `.env` with `CONTROL_APIKEY`.
...
|---------|-------------|------------------|---------------|
| Gluetun | API key (auto-generated) | Environment Variable | `CONTROL_APIKEY` |
