...
try:
    ...
    log("Playwright is not installed. Please run: pip install playwright && playwright install", "ERROR")
    sys.exit(1)
...
def qbit_login(url, username, password):
    try:
        ...
        resp = session.post(f"{url}/api/v2/auth/login", data={"username": username, "password": password})
    ...
    if temp_pass:
        ...
        if session:
            ...
            try:
                ...
                session.post(f"{url}/api/v2/app/setPreferences", data={"json": json.dumps({"bypass_auth_subnet_whitelist_enabled": False})})
                ...
                resp = session.post(f"{url}/api/v2/app/setPreferences", data={"json": json.dumps(payload)})
                
                if resp.status_code == 200:
                    log("qBittorrent credentials updated successfully", "SUCCESS")
                else:
                    log(f"Failed to update qBittorrent credentials: {resp.status_code} {resp.text}", "ERROR")
            except Exception as e:
    ...
    try:
        ...
        if page.is_visible(username_selector) or page.is_visible(username_fallback):
            ...
            if page.is_visible(submit_selector):
                ...
                if page.is_visible(username_selector) or page.is_visible(username_fallback):
                    log(f"Still on login page for {name}. Authentication might have failed or was already set.", "WARNING")
                else:
...
def main():
    ...
    services = [
        ("Prowlarr", os.environ.get("PROWLARR_URL", "http://localhost:9696"), os.environ.get("PROWLARR_PASSWORD")),
        ("Sonarr", os.environ.get("SONARR_URL", "http://localhost:8989"), os.environ.get("SONARR_PASSWORD")),
        ("Radarr", os.environ.get("RADARR_URL", "http://localhost:7878"), os.environ.get("RADARR_PASSWORD")),
        ("Bazarr", os.environ.get("BAZARR_URL", "http://localhost:6767"), os.environ.get("BAZARR_PASSWORD")),
    ]
